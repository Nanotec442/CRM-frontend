...
function ResetPassword() {
  ...
  // 👈 Inicializar estado directamente según el token, sin useEffect
  const [status, setStatus] = useState(() => token ? "idle" : "error");
  const [message, setMessage] = useState(() =>
    token ? "" : "Enlace inválido o expirado. Falta el token de seguridad."
  );
  ...
  const handleSubmit = async (e) => {
    ...
    setMessage("");
    try {
      await resetPassword({ token, nueva_password: formData.password });
      setStatus("success");
