  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-2 duration-300 bg-white p-6 md:p-8 rounded-2xl border border-slate-100 shadow-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        ...
        {/* Correo (Normalmente de solo lectura por seguridad) */}
        <div>
        ...
        {/* Cargo / Rol (Indicador visual basado en base de datos) */}
        <div className="sm:col-span-2">
          <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 ml-1">
            Nivel de Acceso (Rol)
          </label>
          <div className="flex items-center gap-3 p-4 border border-slate-200 rounded-xl bg-slate-50">
            <ShieldCheck size={24} className={form?.is_superadmin ? "text-emerald-500" : "text-slate-400"} />
            <div>
              <p className="text-sm font-bold text-slate-900">
                {form?.is_superadmin ? "Propietario / Super Administrador" : "Usuario Estándar"}
              </p>
              <p className="text-xs text-slate-500 font-medium mt-0.5">
                {form?.is_superadmin 
                  ? "Tienes acceso total a todas las configuraciones del sistema." 
                  : "Tienes permisos limitados según tu rol asignado."}
              </p>
            </div>
          </div>
        </div>
